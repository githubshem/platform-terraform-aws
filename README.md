...
- Top-level folders are `Capitalized`: `Terraform/`, `Jenkins/`, `Scripts/`.
- Everything below them — layers, stacks, modules, leaves — is lowercase
`kebab-case`. The case change is the signal that you have crossed from the
repo's top-level organisation into the Terraform tree.
