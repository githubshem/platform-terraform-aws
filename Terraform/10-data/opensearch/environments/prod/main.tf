...
data "aws_secretsmanager_secret" "os_password" {
  name = "prod/opensearch/tf-os-master-password"
}
...
module "opensearch" {
  ...
  # Index lifecycle management lives in the ism/ workspace beside this one, not
  # here: the opensearch provider has to be configured with this domain's
  # endpoint, and a provider block cannot depend on a resource created in the
  # same apply. Apply this stack first, then ism/.
}